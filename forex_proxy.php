...
function glFetch($url) {
    ...
    $ctx = stream_context_create([
        'http' => ['timeout' => 12, 'header' => "User-Agent: G-Labs/1.0\r\n"],
...
$pr = null;
for ($daysBack = 1; $daysBack <= 4; $daysBack++) {
    $prevDate = date('Y-m-d', strtotime("-{$daysBack} day"));
...
foreach ($curScores as $cur => $moves) {
    ...
    if (count($moves) > 0) $hasAnyChange = true;
}
